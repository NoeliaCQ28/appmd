...
import { TransformadorAccesoriosModal } from "./modals/TransformadorAccesoriosModal";
...
export const ComponentTransformersCard = ({
  title,
  className,
  quoteId,
  quoteDetailId,
  quoteTransformadorId,
  deliveryDays = 0,
  otherComponents,
  quantity = 1,
  isEditMode = false,
  ...
  const { transformersAdded, addTransformer, removeTransformer } =
    ...
    useOtherComponents({
      initialComponents: isEditMode
        ...
        : transformersAdded.find(
            (detail) => detail.nCotDetItem === quoteDetailId
          )?.otherComponents || [],
    });
  ...
  const [totalPrice, setTotalPrice] = React.useState(precioTransformer);
  ...
  const precioTotalBase = React.useMemo(() => {
    return precioTransformer + otherComponentsTotalPrice;
  }, [precioTransformer, otherComponentsTotalPrice]);
        ...
        <ComponentFormFooter
          ...
          onEditAction={() => {
            const payload = {
              ...
              quoteDetailsTransformadorId: quoteTransformadorId,
            ...
            updateDetailOfQuoteMutate(
              ...
              },
              {
                onSuccess: () => {
                  ...
                  setTotalPrice(precioTransformer); // Resetear precio
                  removeTransformer?.(transformerId); // Remover celda si es necesario
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        <TransformadorAccesoriosModal
          ...
          selectedItem={{ TransformadorId: transformerId }}
