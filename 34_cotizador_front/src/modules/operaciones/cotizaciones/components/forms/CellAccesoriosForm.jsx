...
export const CellAccesoriosForm = ({
  detailsAdded,
  addDetail,
  CeldaId,
  addComponent = null,
  ...
  const { accesorios = [], isLoadingAccesorios, errorAccesorios } = useCells();
  ...
  const accesoriosGrouped = React.useMemo(() => {
    ...
    return accesorios.reduce((acc, item) => {
      const key = item.sCelMarDescripcion;
  ...
  const selectedByType = React.useMemo(() => {
    ...
    return accesorios.reduce((acc, item) => {
      if (detailsAdded.some((c) => c.id === item.CeldaAccesorio_Id)) {
        const key = item.sCelMarDescripcion;
        acc[key] = (acc[key] || 0) + 1;
      }
      return acc;
            ...
            {Object.entries(filteredOptionalsGrouped).map(([type, items]) => (
              ...
              >
                ...
                <ul className="space-y-3">
                  {items
                    .sort((a, b) =>
                      a.sCelAccDescripcion.localeCompare(b.sCelAccDescripcion)
                    )
                    .map((item) => {
                      const {
                        CeldaAccesorio_Id,
                      ...
                      const itemMapped = {
                        id: CeldaAccesorio_Id,
                        name: sCelAccDescripcion,
                        description: sCelAccDescripcion,
                        price: Number.parseFloat(nCelAccPrecio),
                      };

                      return (
                        <li
                          key={CeldaAccesorio_Id}
                          className="p-4 border rounded-lg hover:bg-gray-50 transition-colors cursor-pointer"
                          onClick={(e) => {
                            e.stopPropagation();
                            e.preventDefault();
                            // addDetail(CeldaId, itemMapped);
                            // Solo ejecutar addDetail si no estamos en modo edición
                            if (!isEditMode) {
                              addDetail(CeldaId, itemMapped);
                            }
                          }}
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <div className="px-4 py-2 bg-green-50 rounded-lg">
                                <span className="text-xl font-semibold text-green-700">
                                  ${" "}
                                  {Number(nCelAccPrecio).toLocaleString(
                                    "en-US",
                                    {
