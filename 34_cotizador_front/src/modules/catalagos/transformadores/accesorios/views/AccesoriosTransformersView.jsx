...
const AccesoriosTransformersView = () => {
  const {
    accesorios = [],
    isLoadingAccesorios,
    errorAccesorios,
  ...
  const accesoriosFilter = React.useMemo(() => {
    return accesorios.filter((accesorio) => {
      ...
      return (
        accesorio.sTraAccCodSAP?.toString().toLowerCase().includes(query) ||
        accesorio.sTraAccDescripcion
          ?.toString()
          .toLowerCase()
          .includes(query) ||
        accesorio.sTraAccTipo?.toLowerCase().includes(query)
      );
    });
  ...
  const confirmDelete = () => {
    if (selectedItem) {
      deleteAccesorioMutate(selectedItem?.TransformadorAccedorio_Id);
          ...
          <section>
            ...
            >
              <Column
                ...
                sortable
              ></Column>
              <Column field="sTraAccTipo" header="TIPO" sortable></Column>
              <Column
                field="sTraAccDescripcion"
                header="ACCESORIO"
                sortable
              ></Column>
              <Column
...
export default AccesoriosTransformersView;
